...
function Tree(target = {}, desc) {
    ...
    const tree = {
        ...
        resolve(path) {
            return Path.resolve(path, target);
        },
        ...
        get(path, view) {
            return Path.get(path, target, view);
        },

        set(path, value) {
            return Path.set(path, target, value);
        },

        update(path, method, args, options) {
            Path.update(path, target, method, args, options);
        ...
        apply(path, func, ...args) {
            ...
            Path.update(path, target, method, args);
