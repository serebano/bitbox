...
function curry(fn) {
    ...
    if (arguments.length >= 2) {
        if (is.integer(arguments[1])) {
            length = arguments[1]
        } else if (is.placeholder(arguments[1])) {
            length = arguments.length - 1
            //return curry(fn, arguments.length - 1)(...Array.prototype.slice.call(arguments, 1))
        }
    }
    ...
    const fna = arity(length, function __fn1(...args) {
        ...
        if (shortfall <= 0) {
            console.warn(`shortfall`, shortfall, args)
            return fn(...args)
        }
